...
<html>
	<head>
		<meta charset="UTF-8">
		<title>码客帮 - 让靠谱的工程师为你开发 | 互联网软件众包平台 - ${provinceNode.name}</title>
		<meta name="keywords" content="码客帮, 软件外包, 软件众包, 代码众包, 开发者众包, 程序员兼职, 项目外包,互联网软件外包,网站外包,安卓APP外包,Andriod APP,iOS APP外包,微信开发外包,${provinceNode.name}">
    ...
    <script type="text/javascript" src="${ctx}/js/header.js?v=201504142230"></script>
		<link rel="stylesheet" href="${ctx}/css/infos/province.css"/>
    ...
    <#include "../header.ftl">
		...
		<div class="body">
				...
				<div class="BreadcrumbNavigation">
					<a href="/infos/index">首页</a><i class="fa fa-angle-right"></i>
					<a href="#">${provinceNode.name!""}</a>
				</div>
		 ...
		 <#if provinceNode.name?index_of("北京") == -1 &&
				 	provinceNode.name?index_of("重庆") == -1 &&
				  provinceNode.name?index_of("上海") == -1 &&
				  provinceNode.name?index_of("天津") == -1>
				<div class="region">
					<div>地区：</div>
					<ul>
						<#list cityNodes as cityNode>
								<li><a href="/infos/${provinceNode.urlId!""}/${cityNode.urlId}">${cityNode.name}</a></li>
						</#list>
					</ul>
				</div>
			</#if>
			...
			<div class="body_left">
				...
				<div class="content">
					<#list provinceNode.items as item>
					<div class="article">
				...
				<ul>
					<#list siblingNodes as siblingNode>
						<li>
							...
							<a href="/infos/${siblingNode.urlId!""}">${siblingNode.name}</a>
						</li>
					</#list>
				</ul>
			</div>
